...
export const metadata: Metadata = {
  title: "コインランドリー充実度ランキング｜市区町村別",
  description:
    "コインランドリーの口コミをAIが解析し、24時間営業・大型洗濯機の両方が確認できた店舗の割合が高い市区町村をランキング形式で紹介します。",
};
...
async function fetchRanking(): Promise<CityStats[]> {
  ...
  for (const row of rows) {
    ...
    if (isLaundryMetadata(metadata) && metadata.has_24h && metadata.has_large_machine) {
      entry.ready += 1;
    }
...
export default async function LaundryRankingPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      ...
      <p className="mt-3 text-sm text-gray-600">
        各市区町村のコインランドリーのうち、口コミのAI解析で24時間営業・大型洗濯機/乾燥機の両方が確認できた店舗の割合が高い順に並べています。
      </p>
      <RankingCrossLinks current="laundry" />

      {stats.length > 0 && (
        <Link
          href={`/${encodeURIComponent(stats[0].prefecture)}/${encodeURIComponent(stats[0].city)}/laundry`}
          ...
          {stats.map((stat, index) => (
            <li key={`${stat.prefecture}-${stat.city}`} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <Link
                  href={`/${encodeURIComponent(stat.prefecture)}/${encodeURIComponent(stat.city)}/laundry`}
                  className="truncate font-medium text-indigo-600 hover:underline"
                >
                  {stat.prefecture}{stat.city}
                </Link>
                <p className="mt-0.5 text-xs text-gray-500">
                  調査{stat.total}件中{stat.ready}件で24時間営業+大型機を確認
                </p>
              </div>
