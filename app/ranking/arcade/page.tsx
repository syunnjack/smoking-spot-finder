...
import { isArcadeMetadata, parseVenueMetadata } from "@/lib/types";
...
export const metadata: Metadata = {
  title: "ゲームセンター プリクラ・クレーンゲーム充実度ランキング｜市区町村別",
  description:
    "ゲームセンターの口コミをAIが解析し、プリクラ・クレーンゲームの両方が確認できた店舗の割合が高い市区町村をランキング形式で紹介します。",
};
...
async function fetchRanking(): Promise<CityStats[]> {
  ...
  for (const row of rows) {
    ...
    const entry = byCity.get(key) ?? { prefecture, city, total: 0, ready: 0, rate: 0 };

    entry.total += 1;
    const metadata = parseVenueMetadata(row.metadata);
    if (isArcadeMetadata(metadata) && metadata.has_crane_game && metadata.has_purikura) {
      entry.ready += 1;
    }
  ...
  const stats = [...byCity.values()].map((entry) => ({
    ...entry,
    rate: entry.total > 0 ? entry.ready / entry.total : 0,
  }));
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      ...
      <h1 className="mt-4 text-2xl font-bold text-gray-900 sm:text-3xl">
        ゲームセンター プリクラ・クレーンゲーム充実度ランキング
      </h1>
      <p className="mt-3 text-sm text-gray-600">
        各市区町村のゲームセンターのうち、口コミのAI解析でクレーンゲーム・プリクラの両方が確認できた店舗の割合が高い順に並べています。
      </p>
      <RankingCrossLinks current="arcade" />

      {stats.length > 0 && (
        <Link
          href={`/${encodeURIComponent(stats[0].prefecture)}/${encodeURIComponent(stats[0].city)}/arcade`}
        ...
        >
          <span className="text-sm text-gray-700">
            ...
            (充実率{Math.round(stats[0].rate * 100)}%)
          </span>
          ...
          {stats.map((stat, index) => (
            <li key={`${stat.prefecture}-${stat.city}`} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <Link
                  href={`/${encodeURIComponent(stat.prefecture)}/${encodeURIComponent(stat.city)}/arcade`}
                  className="truncate font-medium text-indigo-600 hover:underline"
                >
                  {stat.prefecture}{stat.city}
                </Link>
                <p className="mt-0.5 text-xs text-gray-500">
                  調査{stat.total}件中{stat.ready}件でクレーンゲーム+プリクラを確認
                </p>
              </div>
